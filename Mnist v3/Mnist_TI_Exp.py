...
def plot_batch_losses(batch_losses, save_path="batch_losses_exp.png"):
...
def main():
    parser = argparse.ArgumentParser(description='PyTorch Transitive Inference with Exception')
    ...
    parser.add_argument('--exception-winner', type=int, default=5)
    parser.add_argument('--exception-loser', type=int, default=2)
    ...
    exception_pair = (args.exception_winner, args.exception_loser)
    print(f"Exception pair: {exception_pair[0]} beats {exception_pair[1]}")

    train_dataset = TransitiveTrainDataset_Exp(mnist_train, n=8, seed=args.data_seed,
                                                exception_pair=exception_pair)
    ...
    plot_batch_losses(batch_losses, save_path="batch_losses_exp.png")

    if args.save_model:
        torch.save(model.state_dict(), "ti_exp_cnn.pt")
