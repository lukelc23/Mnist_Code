...
dataset=$1
script="$HOME/Projects/Mnist_v7_sbatch/Mnist_TI.py"
origin_folder="/mnt/smb/locker/abbott-locker/Luke/Mnist/Mnist_v7_sbatch"
n_items=9
exception_pair="5 3"
    ...
    do
        ...
        do
            ...
            do
                ...
                echo "${script} --output-folder ${output_folder} --exception ${exception} --dropout ${dropout} --intermediate-layer ${intermediate_layer} --ordering-seed ${ordering_seed} --n-items ${n_items} --epochs 100 ${exception_pair_arg}" >> ${dataset}_jobs.txt
            done
        done
    done
done